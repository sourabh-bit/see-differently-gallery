      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <motion.div style={{ x }} className="flex h-full will-change-transform">
          {panels.map((p, i) => (
            <div
              key={p.n}
              className="w-screen h-full flex-shrink-0 grid grid-cols-12 items-center px-6 md:px-20 gap-8"
            >
              <div className="col-span-12 md:col-span-5 flex flex-col gap-6">
                <div className="serif italic text-2xl text-graphite">{p.n}.</div>
                <h3 className="serif text-[18vw] md:text-[12vw] leading-[0.85] tracking-[-0.03em]">
                  {p.word}.
                </h3>
                <p className="text-graphite max-w-sm text-base md:text-lg">{p.caption}</p>
                <div className="mono text-[10px] tracking-[0.3em] uppercase text-graphite mt-4">
                  Hour {i + 1} of 7
                </div>
              </div>
              <div className="hidden md:block col-span-6 col-start-7 h-[70vh]" data-cursor="image">
                <img
                  src={p.img}
                  alt={p.word}
                  width={1280}
