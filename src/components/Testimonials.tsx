...
export function Testimonials() {
  return (
    <section className="bg-bone py-32 md:py-44 px-6 md:px-12 border-y border-ink/10">
      <div className="max-w-7xl mx-auto">
        <div className="mono text-[11px] tracking-[0.3em] uppercase text-graphite mb-16 flex items-center gap-4">
          <span className="w-8 h-px bg-ink" /> 05 — Voices
        </div>
        <div className="grid md:grid-cols-3 gap-12 md:gap-20">
          {items.map((t, i) => (
            <figure key={i} className="flex flex-col gap-8">
              <div className="serif text-[10vw] md:text-7xl leading-none text-ink/80">"</div>
              <blockquote className="serif italic text-2xl md:text-3xl leading-snug">
                {t.quote}
              </blockquote>
